...
using System;
using System.Linq;
namespace newAnimationAystem{
    public class ColorAnimationSystem  : MonoBehaviour {
    ...
    public class AnimationPrototype
    {
        ...
        [HideInInspector]public float center;
    ...
    public class LightingUpFromStartToEnd : AnimationPrototype
    {
        ...
        void AnimatiionLogic_1(bool _isUseAntiOrder,int maxDarkNumber, ref bool[] _states)
        {
            ...
            min = (_isUseAntiOrder ? (_states.Length - 1) + maxDarkNumber : 0 - maxDarkNumber) + timeIndex;
            max = (_isUseAntiOrder ? min - (maxDarkNumber - 1) : min + (maxDarkNumber - 1));
            center = (max + min)/2 ;
